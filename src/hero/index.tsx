...
const getScrollLeft = () => document.documentElement.scrollLeft || document.body.scrollLeft;
...
const diffStyle = (el: HTMLDivElement, styles: Record<string, any>, updateStyles: Record<string, any>) => {
    for(const key in styles) {
        if (updateStyles[key]) {
            el.style[key as any] = updateStyles[key]
        } else {
            el.style[key as any] = ''
        }
    }
...
const getHero = (key: string) => heroMap.get(key);

const initCurrentHero = (key: string, props: { duration: number, zIndex: number, timing: string }) => {
    heroMap.set(key, props)
    return props;
}

const initSecondHero = (key: string, animateEndCallback: Function) => {
    const currentHero = getHero(key);
...
const renderChangeAnimate = (key: string) => {
    const currentHero = getHero(key);
    const {
        $last,
        firstOffset,
    ...
    const newEl = setElementStyle($last.cloneNode(true), {
        left: `${firstOffset.left}px`,
        top: `${firstOffset.top}px`,
        position: 'absolute',
        transition: `${duration}s all ${timing}`,
        zIndex: zIndex + 1,
        opacity: 0,
    })
    currentHero.transHeroElLast = newEl;
    diffStyle(newEl, lastStyles, firstStyles)
    ...
    setTimeout(() => {
        ...
        for(const key in lastStyles) {
            transHeroEl.style[key] = lastStyles[key]
            newEl.style[key as any] = lastStyles[key]
        }
    }, 10)
}

const destoryChangeAnimate = (key: string) => {
    const currentHero = getHero(key);
...
export const Hero = defineComponent((props: {
    k: string;
    duration?: number;
    zIndex?: number;
    timing?: string;
    ...
    if (currentHero) {
        ...
        initSecondHero(props.k, () => loadSuccess.value = true);
    } else {
        currentHero = initCurrentHero(props.k, {
